...
module.exports = async (client) => {
  ...
  await createStructuredTextFieldFrom(client, "article", "body", 
    modelIds.article.fields,
  );
  const records = await getAllRecords(client, "article");

  console.log("records ", records)

  for (const record of records) {
    await client.items.update(record.id, {
      structuredTextContent: await htmlToStructuredText(record.body, {
        preprocess: (tree) => {
  ...
  await swapFields(client, "article", "body");
};